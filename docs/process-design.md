...
| エラー種別 | 対応方針 |
|-----------|---------|
| バリデーションエラー | フィールド単位でエラーメッセージを表示。入力内容は保持する |
| 権限エラー | 「この操作を行う権限がありません」メッセージを表示 |
| ステータス不整合 | 「この申請は既に処理されています」メッセージを表示し、一覧に戻す |
| ファイルアップロードエラー | ファイル形式・サイズのエラーメッセージを表示 |
| サーバーエラー | 汎用エラーメッセージを表示。詳細はサーバーログに記録 |
